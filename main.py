...
# @app.get("/current-months-read-pages")
# def delete_current_month_pages_read(db: Session = Depends(get_db)):
#     query = select(YearlyPageCount)
#     yearly_page_counts = db.scalars(query).all()
#     for yearly_page_count in yearly_page_counts:
#         return getattr(yearly_page_count, datetime.now().strftime('%B').lower())

    # for yearly_page_count in yearly_page_counts:
    #     if getattr(yearly_page_count, datetime.now().strftime('%B').lower()):
    #         return setattr(yearly_page_count, datetime.now().strftime('%B').lower(), 0)
...
# @app.get("/current-months-read-pages")
# def get_current_month_pages_read(db: Session = Depends(get_db)):
...
#     users_pages_read = {}
#     for yearly_page_count in yearly_page_counts:
#         user_id = yearly_page_count.user_id
#         pages_read = getattr(yearly_page_count, current_month)
#         users_pages_read[user_id] = pages_read

#     return users_pages_read


# @app.delete("/delete-current-months-read-pages")
# def delete_current_month_pages_read(db: Session = Depends(get_db)):
#     current_month = datetime.now().strftime('%B').lower()
#     delete_query = delete(YearlyPageCount).where(getattr(YearlyPageCount, current_month) > 0)
